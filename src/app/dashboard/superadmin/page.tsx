...
export default function SuperAdminDashboardPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [orgsData, incidentsRes] = await Promise.all([
    ...
    const getSubscriptionBadge = (plan: string) => {
        const styles: Record<string, string> = {
            ...
            PROFESSIONAL: 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800',
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                {mainStats.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className="group bg-surface overflow-hidden rounded-xl p-5 shadow-sm border border-glass transition-all duration-300 hover:shadow-lg hover:border-purple-500/30"
                    >
            ...
            <DashboardCharts
                stats={stats}
                incidentStats={stats.incidentBreakdown}
            />
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                <div className="bg-surface rounded-xl border border-glass shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-glass flex items-center justify-between">
                        ...
                        <Link href="/dashboard/superadmin/organizations" className="text-sm text-purple-500 hover:text-purple-600 flex items-center gap-1 font-medium">
                            ...
                            recentOrgs.map((org) => (
                                <div key={org.organizationId} className="p-4 flex items-center justify-between hover:bg-glass/50 transition-colors">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${org.isActive ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-600' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'}`}>
                                            <Building2 size={18} />
                                        </div>
                                        <div>
